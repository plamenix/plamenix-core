        ...
        match sqltype {
            ...
            ibase::SQL_INT64 => {
                ...
                if null {
                    ...
                    // PLAMENIX PATCH: a non-zero scale means NUMERIC or
                    // DECIMAL, which `coerce` no longer turns into a
                    // double. `SqlType` has no exact fixed-point variant,
                    // so surface the exact decimal as text — the same
                    // treatment the native backend already gives INT128
                    // and DECFLOAT. `raw_type` stays SQL_INT64, so a
                    // caller can tell this apart from a real VARCHAR.
                    data.push(ParsedColumn::Complete(Column::new(
                        var.alias_name.clone(),
                        sqltype,
                        SqlType::Text(scaled_to_decimal_string(i, var.scale)),
                    )))
                } else {
                    data.push(ParsedColumn::Complete(Column::new(
                        var.alias_name.clone(),
...
/// Renders a Firebird scaled integer as exact decimal text.
///
/// PLAMENIX PATCH. Firebird stores `NUMERIC(p,s)` and `DECIMAL(p,s)` as
/// an integer plus a scale, where the real value is
/// `unscaled * 10^scale` and `scale` is negative for the fractional
/// case. Upstream sidesteps the representation by asking the engine for
/// a double, which is lossy; this keeps every digit.
fn scaled_to_decimal_string(unscaled: i64, scale: i16) -> String {
    if scale == 0 {
...
#[cfg(test)]
mod plamenix_numeric_tests {
    use super::scaled_to_decimal_string;

    #[test]
    fn renders_fractional_scales_exactly() {
        // NUMERIC(18,4) holding 12.3456
        assert_eq!(scaled_to_decimal_string(123_456, -4), "12.3456");
        // Values smaller than the scale need a zero integer part.
        assert_eq!(scaled_to_decimal_string(5, -4), "0.0005");
        assert_eq!(scaled_to_decimal_string(-5, -4), "-0.0005");
        assert_eq!(scaled_to_decimal_string(0, -2), "0.00");
    }

    #[test]
    fn keeps_precision_a_double_would_lose() {
        // 99999999999999.9999 as NUMERIC(18,4): the scaled integer is
        // past 2^53, so the upstream f64 path cannot represent it.
        assert_eq!(
            scaled_to_decimal_string(999_999_999_999_999_999, -4),
            "99999999999999.9999"
        );
    }

    #[test]
    fn handles_the_range_ends() {
        assert_eq!(scaled_to_decimal_string(i64::MAX, 0), i64::MAX.to_string());
        assert_eq!(
            scaled_to_decimal_string(i64::MIN, -2),
    ...
    fn positive_scale_multiplies_rather_than_truncating() {
        assert_eq!(scaled_to_decimal_string(5, 2), "500");
        assert_eq!(scaled_to_decimal_string(-5, 2), "-500");
    }
}
